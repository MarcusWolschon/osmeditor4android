...
public class UserAccountTest {
    ...
    public void userAccount() {
        ...
        assertTrue(TestUtils.clickText(device, false, main.getString(R.string.config_loginbutton_title), true, false));
        assertTrue(TestUtils.findText(device, false, main.getString(R.string.config_loginbutton_title)));

        assertTrue(TestUtils.clickText(device, false, main.getString(R.string.config_username_title), true, false));
        UiObject name = TestUtils.findObjectWithResourceId(device, false, device.getCurrentPackageName() + ":id/loginedit_editUsername");
        try {
            name.setText("test");
        } catch (UiObjectNotFoundException e) {
            fail(e.getMessage());
        }
        UiObject password = TestUtils.findObjectWithResourceId(device, false, device.getCurrentPackageName() + ":id/loginedit_editPassword");
        try {
            password.clearTextField();
            password.setText("tset");
        } catch (UiObjectNotFoundException e) {
            fail(e.getMessage());
        }
        ...
        try (AdvancedPrefDatabase prefDb = new AdvancedPrefDatabase(main)) {
            ...
            assertEquals("tset", current.pass);
        }
    }
}