...
public class GpxTest {
    ...
    public void recordSaveAndImportGpx() {
        ...
        for (MapViewLayer l : main.getMap().getLayers()) {
            if (l instanceof io.vespucci.layer.gpx.MapOverlay && filename.equals(((io.vespucci.layer.gpx.MapOverlay) l).getName())) {
                recordedTrack = ((io.vespucci.layer.gpx.MapOverlay) l).getTrack().getTrackPoints();
            }
        }
    ...
    public void importWayPoints() {
        try {
            ...
            for (MapViewLayer layer : main.getMap().getLayers()) {
                if (layer instanceof io.vespucci.layer.gpx.MapOverlay && GpxUploadTest.GPX_FILE.equals(layer.getName())) {
    ...
    public void importWayPointsWithLinks() {
        try {
            ...
            for (MapViewLayer layer : main.getMap().getLayers()) {
                if (layer instanceof io.vespucci.layer.gpx.MapOverlay && "2011-06-08_13-21-55.gpx".equals(layer.getName())) {
                    foundLayer = layer;
                    break;
                }
            }
            ...
            Track track = ((io.vespucci.layer.gpx.MapOverlay)foundLayer).getTrack();
