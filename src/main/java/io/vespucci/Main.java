...
import io.vespucci.dialogs.BarometerCalibration;
import io.vespucci.dialogs.ConsoleDialog;
import io.vespucci.dialogs.DataLoss;
import io.vespucci.dialogs.DownloadCurrentWithChanges;
import io.vespucci.dialogs.ElementInfo;
import io.vespucci.dialogs.GnssPositionInfo;
import io.vespucci.dialogs.Layers;
import io.vespucci.dialogs.NewVersion;
import io.vespucci.dialogs.Newbie;
import io.vespucci.dialogs.Progress;
import io.vespucci.dialogs.Review;
import io.vespucci.dialogs.ReviewAndUpload;
import io.vespucci.dialogs.SearchForm;
import io.vespucci.dialogs.Tip;
import io.vespucci.dialogs.TooMuchData;
import io.vespucci.dialogs.UndoDialog;
import io.vespucci.dialogs.bookmarks.BookmarkEdit;
import io.vespucci.dialogs.bookmarks.BookmarksDialog;
import io.vespucci.easyedit.EasyEditManager;
import io.vespucci.easyedit.ElementSelectionActionModeCallback;
import io.vespucci.easyedit.MultiSelectActionModeCallback;
import io.vespucci.easyedit.SimpleActionModeCallback;
...
import io.vespucci.filter.TagFilter;
import io.vespucci.geocode.GeocodeInput;
import io.vespucci.geocode.Search.SearchResult;
import io.vespucci.gpx.TrackPoint;
import io.vespucci.imageryoffset.ImageryAlignmentActionModeCallback;
...
import io.vespucci.layer.DownloadInterface;
import io.vespucci.layer.LayerType;
import io.vespucci.layer.MapViewLayer;
import io.vespucci.layer.geojson.MapOverlay;
import io.vespucci.layer.streetlevel.NetworkImageLoader;
import io.vespucci.layer.streetlevel.SelectImageInterface;
import io.vespucci.layer.tiles.MapTilesLayer;
import io.vespucci.listener.UpdateViewListener;
...
import io.vespucci.resources.TileLayerDatabaseView;
import io.vespucci.resources.TileLayerSource;
import io.vespucci.search.Search;
import io.vespucci.sensors.CompassEventListener;
import io.vespucci.services.TrackerService;
import io.vespucci.services.TrackerService.TrackerBinder;
import io.vespucci.services.TrackerService.TrackerLocationListener;
import io.vespucci.tasks.MapRouletteApiKey;
...
import io.vespucci.util.FullScreenAppCompatActivity;
...
import io.vespucci.views.ZoomControls;
...
public class Main extends FullScreenAppCompatActivity
        implements ServiceConnection, TrackerLocationListener, UpdateViewListener, io.vespucci.geocode.SearchItemSelectedCallback, ActivityResultHandler {
    ...
    private io.vespucci.views.ZoomControls zoomControls;
    ...
    protected void onResume() {
        ...
        PostAsyncActionHandler postLoadData = () -> {
            ...
            io.vespucci.layer.data.MapOverlay<OsmElement> dataLayer = map.getDataLayer();
            if (dataLayer != null) {
        ...
        PostAsyncActionHandler postLoadTasks = () -> {
            ...
            io.vespucci.layer.tasks.MapOverlay layer = map.getTaskLayer();
            if (layer != null) {
        ...
        synchronized (newIntentsLock) {
            while ((intent = newIntents.poll()) != null) {
                ...
                case ACTION_DELETE_PHOTO: // harmless as this just deletes from the index
                    ...
                    final io.vespucci.layer.photos.MapOverlay photoLayer = map != null ? map.getPhotoLayer() : null;
                    if (photoLayer != null) {
    ...
    private void loadGeoJson() {
        io.vespucci.layer.Util.addLayer(this, LayerType.GEOJSON, contentUri.toString());
        map.setUpLayers(this);
        io.vespucci.layer.geojson.MapOverlay layer = (MapOverlay) map.getLayer(LayerType.GEOJSON, contentUri.toString());
        if (layer != null) {
    ...
    private void handlePhotoUri() {
        if (map.getPhotoLayer() == null) {
            io.vespucci.layer.Util.addLayer(this, LayerType.PHOTO);
            getMap().setUpLayers(this);
            io.vespucci.layer.photos.MapOverlay photoLayer = map.getPhotoLayer();
            if (photoLayer != null) {
    ...
    private void loadGPXFile(@NonNull final Uri uri) {
        io.vespucci.layer.gpx.MapOverlay gpxLayer = (io.vespucci.layer.gpx.MapOverlay) map.getLayer(LayerType.GPX, uri.toString());
        if (gpxLayer == null) {
            io.vespucci.layer.Util.addLayer(this, LayerType.GPX, uri.toString());
            map.setUpLayers(this);
            gpxLayer = (io.vespucci.layer.gpx.MapOverlay) map.getLayer(LayerType.GPX, uri.toString());
    ...
    public void downLoadBugs(BoundingBox bbox) {
        if (isConnected()) { // don't try if we are not connected
            ...
            TransferTasks.downloadBox(this, prefs.getServer(), bbox, true, TransferTasks.MAX_PER_REQUEST, () -> {
                io.vespucci.layer.tasks.MapOverlay taskLayer = map.getTaskLayer();
                if (taskLayer != null) {
    ...
    public static void showOverpassConsole(@NonNull final FragmentActivity activity, @Nullable String text) {
        ConsoleDialog.showDialog(activity, R.string.overpass_console, R.string.merge_result, R.string.select_result, text, null,
                (context, input, merge, select) -> {
                    ...
                    AsyncResult result = io.vespucci.overpass.Server.query(context, io.vespucci.overpass.Server.replacePlaceholders(context, input),
                            merge, select);
    ...
    public static void showJsConsole(@NonNull final Main main) {
        ...
        ConsoleDialog.showDialog(main, R.string.tag_menu_js_console, -1, -1, null, null, (context, input, flag1, flag2) -> {
            String result = io.vespucci.javascript.Utils.evalString(context, "JS Console", input, App.getLogic());
            if (context instanceof Main) {
            ...
            if (map.getLayer(LayerType.GPX, trackingLayer) == null) { // not displayed
                try (AdvancedPrefDatabase db = new AdvancedPrefDatabase(this)) { // not configured
                    if (!db.hasLayer(LayerType.GPX, trackingLayer)) {
                        io.vespucci.layer.Util.addLayer(this, LayerType.GPX, trackingLayer);
                    }
                    map.setUpLayers(this);
                }
            }
    ...
    public void onServiceConnected(ComponentName name, IBinder service) {
        ...
        if (TrackerService.class.getCanonicalName().equals(name.getClassName())) {
            ...
            io.vespucci.layer.gpx.MapOverlay layer = (io.vespucci.layer.gpx.MapOverlay) map.getLayer(LayerType.GPX,
