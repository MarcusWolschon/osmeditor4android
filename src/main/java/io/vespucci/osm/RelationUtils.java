...
import static io.vespucci.util.Geometry.isInside;
...
public final class RelationUtils {
     ...
     */
    @NonNull
    public static int[] calcCentroid(@NonNull Relation r, @NonNull io.vespucci.Map map, @NonNull ViewBox viewBox) {
        long latE7 = 0;
        long lonE7 = 0;
        List<List<RelationMember>> rings = new ArrayList<>();
    ...
    private static double minDistanceToRelation(@NonNull List<OsmElement> selection, @NonNull Relation r) {
        ...
        for (OsmElement e : selection) {
            ...
            double[] centroid = io.vespucci.util.Geometry.centroid(e);
            if (centroid.length == 2) {
