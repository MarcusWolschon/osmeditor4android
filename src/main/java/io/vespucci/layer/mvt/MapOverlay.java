...
import io.vespucci.layer.tiles.util.MapTileProvider;
...
public class MapOverlay extends MapTilesOverlayLayer<java.util.Map<String, List<VectorTileDecoder.Feature>>>
    ...
    private double distanceToLineString(final float x, final float y, List<Point> vertices, float tolerance) {
        ...
        for (int k = 0, verticesSize = vertices.size(); k < verticesSize - 1; ++k) {
            ...
            double distance = io.vespucci.util.Geometry.isPositionOnLine(tolerance / 2, x, y, p1X, p1Y, p2X, p2Y);
            if (distance >= 0) {
                return distance;
            }
            p1X = p2X;
    ...
    @Override
    public void onSelected(FragmentActivity activity, io.vespucci.util.mvt.VectorTileDecoder.Feature f) {
    ...
    @Override
    public io.vespucci.util.mvt.VectorTileDecoder.Feature getSelected() {
        return null;
    }

    @Override
    public void setSelected(io.vespucci.util.mvt.VectorTileDecoder.Feature o) {
        // Empty
    }

    @Override
    public SpannableString getDescription(io.vespucci.util.mvt.VectorTileDecoder.Feature f) {
        return getDescription(map.getContext(), f);
    }

    @Override
    public SpannableString getDescription(Context context, io.vespucci.util.mvt.VectorTileDecoder.Feature f) {
