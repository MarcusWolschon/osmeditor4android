...
public class BoundingBox implements Serializable, JosmXmlSerializable, BoundedObject {
     ...
     */
    public BoundingBox(final double left, final double bottom, final double right, final double top) throws OsmException {
    ...
    public void setRatio(Map map, final float ratio, final boolean preserveZoom) throws OsmException {
        long mTop = GeoMath.latE7ToMercatorE7(top); // note long or else we get an int overflow on calculating the
                                                    // center
        ...
        if (width <= 0 || mHeight <= 0) {
            ...
            BoundingBox bbox = GeoMath.createBoundingBoxForCoordinates(GeoMath.mercatorE7ToLat((int) (mBottom + mHeight / 2)),
                    GeoMath.mercatorE7ToLat((int) (left + width / 2)), 10.0f, true);
            left = bbox.left;
            bottom = bbox.bottom;
            right = bbox.right;
            top = bbox.top;
            ...
            for (BoundingBox rb : result) {
                if (b.intersects(rb)) {
                    try {
                        ...
                        Log.d(DEBUG_TAG, "newBoxes " + e.getMessage());
                    }
                } else {
                    temp.add(rb);
                }
