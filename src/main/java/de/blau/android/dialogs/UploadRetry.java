...
public class UploadRetry extends ImmersiveDialogFragment {
    ...
    private void determineStatus(@NonNull final Preferences prefs, @NonNull final Logic logic, @NonNull final long changesetId, @NonNull AlertDialog dialog) {
        new ExecutorTask<Long, Void, Integer>(logic.getExecutorService(), logic.getHandler()) {
            ...
            public void setErrorText(@NonNull String message) {
                dialog.setMessage(message);
                setupButton(dialog.getButton(DialogInterface.BUTTON_NEUTRAL), R.string.save_changes,
                        (View v) -> Main.saveOscFile(getActivity(), App.getDelegator(), prefs));
            }

            @Override
            protected void onPostExecute(Integer result) {
                Progress.dismissDialog(getActivity(), Progress.PROGRESS_UPLOADING, PROGRESS_STATUS_TAG);
                if (result == null) {
                    setErrorText("Unexpected error trying to determine status of changeset " + changesetId + ".");
                    return;
                }
                final Button neutral = dialog.getButton(DialogInterface.BUTTON_NEUTRAL);
                if (result == NO_CHANGES_UPLOADED) {
                    dialog.setMessage(getString(R.string.upload_retry_message_nothing_uploaded, changesetId));
                    ...
                    setupButton(neutral, R.string.retry, (View v) -> logic.upload(getActivity(), arguments, null));
                    return;
                }
                if (result == ALL_CHANGES_UPLOADED) {
                    dialog.setMessage(getString(R.string.upload_retry_message_all_uploaded, changesetId));
                    setupButton(neutral, R.string.patch_data, (View v) -> patchData(delegator, storage));
                ...
                if (result == CHANGES_PARTIALLY_UPLOADED) {
                    dialog.setMessage(getString(R.string.upload_retry_message_partial_upload, changesetId));
                    setupButton(neutral, R.string.patch_data, (View v) -> patchData(delegator, storage));
                }
            }
     ...
     */
    private void patchData(@NonNull final StorageDelegator delegator, @NonNull final Storage storage) {
        ExecutorTask<Void, Void, Boolean> task = new ExecutorTask<Void, Void, Boolean>() {
            ...
            protected Boolean doInBackground(Void v) throws NumberFormatException, XmlPullParserException, IOException {
                ...
                return delegator.updateDataFromChanges(storage);
            }
