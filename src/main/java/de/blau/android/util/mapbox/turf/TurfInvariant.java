...
import de.blau.android.util.mapbox.geojson.GeoJSON;
...
/**
 * Also called Assertions, these methods enforce expectations of a certain type or calculate various shapes from given
 * points.
 *
...
class TurfInvariant {
    ...
    public static void geojsonType(GeoJSON value, String type, String name) throws TurfException {
        ...
        if (value == null || !value.getType().equals(type)) {
            throw new TurfException("Invalid input to " + name + ": must be a " + type + ", given " + (value != null ? value.getType() : " null"));
        }
    }
    ...
    /**
     * Enforce expectations about types of {@link FeatureCollection} inputs for Turf. Internally this uses
     * {@link Feature#getType()}} to judge geometry types.
     *
