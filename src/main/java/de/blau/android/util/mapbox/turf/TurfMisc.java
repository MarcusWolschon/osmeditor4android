...
class TurfMisc {
    ...
    /**
     * Takes a line, a start {@link Point}, and a stop point and returns the line in between those points.
     *
    ...
    private static Feature pointOnLine(Point pt, List<Position> coords) throws TurfException {
        ...
        Feature closestPt = Feature.fromGeometry(Point.fromCoordinates(Position.fromCoordinates(Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY)));
        ...
        for (int i = 0; i < coords.size() - 1; i++) {
            ...
            Feature perpendicularPt = Feature.fromGeometry(TurfMeasurement.destination(pt, 1000, direction + 90, units)); // 1000
                                                                                                                          // =
                                                                                                                          // gross
            LineIntersectsResult intersect = lineIntersects(pt.getCoordinates().getLongitude(), pt.getCoordinates().getLatitude(),
                    ((Point) perpendicularPt.getGeometry()).getCoordinates().getLongitude(),
                    ((Point) perpendicularPt.getGeometry()).getCoordinates().getLatitude(), ((Point) start.getGeometry()).getCoordinates().getLongitude(),
                    ((Point) start.getGeometry()).getCoordinates().getLatitude(), ((Point) stop.getGeometry()).getCoordinates().getLongitude(),
                    ((Point) stop.getGeometry()).getCoordinates().getLatitude());
            if (intersect == null) {
                perpendicularPt = Feature.fromGeometry(TurfMeasurement.destination(pt, 1000, direction - 90, units)); // 1000
                                                                                                                      // =
                                                                                                                      // gross
                intersect = lineIntersects(pt.getCoordinates().getLongitude(), pt.getCoordinates().getLatitude(),
                        ((Point) perpendicularPt.getGeometry()).getCoordinates().getLongitude(),
                        ((Point) perpendicularPt.getGeometry()).getCoordinates().getLatitude(), ((Point) start.getGeometry()).getCoordinates().getLongitude(),
                        ((Point) start.getGeometry()).getCoordinates().getLatitude(), ((Point) stop.getGeometry()).getCoordinates().getLongitude(),
                        ((Point) stop.getGeometry()).getCoordinates().getLatitude());
            ...
            if (intersectPt != null && (double) intersectPt.getNumberProperty("dist") < (double) closestPt.getNumberProperty("dist")) {
    ...
    private static LineIntersectsResult lineIntersects(double line1StartX, double line1StartY, double line1EndX, double line1EndY, double line2StartX,
            double line2StartY, double line2EndX, double line2EndY) {
