package de.blau.android.imagestore;
...
import de.blau.android.prefs.ImageStoreConfiguraton;
...
public class PanoramaxStore implements ImageStore {

    private static final int    TAG_LEN   = Math.min(LOG_TAG_LEN, PanoramaxStore.class.getSimpleName().length());
    private static final String DEBUG_TAG = PanoramaxStore.class.getSimpleName().substring(0, TAG_LEN);
    ...
    private final ImageStoreConfiguraton configuration;
    private final OkHttpClient           client;

    public PanoramaxStore(ImageStoreConfiguraton configuration) {
                ...
                protected Boolean doInBackground(Void nothing) throws XmlPullParserException, IOException {
                    ...
                    try (Response generateKeyCallResponse = client.newCall(generateKeyRequest).execute()) {
                        ...
                        try (KeyDatabaseHelper kdb = new KeyDatabaseHelper(context); SQLiteDatabase db = kdb.getWritableDatabase()) {
                            KeyDatabaseHelper.replaceOrDeleteKey(db, configuration.name, KeyDatabaseHelper.EntryType.PANORAMAX_KEY, key, false, true, null,
                                    null);
                        }
    ...
    private String getKey(@NonNull Context context) {
        try (KeyDatabaseHelper kdb = new KeyDatabaseHelper(context); SQLiteDatabase db = kdb.getWritableDatabase()) {
            return KeyDatabaseHelper.getKey(db, configuration.name, EntryType.PANORAMAX_KEY);
        }
    }
    ...
    public UploadResult upload(Context context, File imageFile) {
        try {
            ...
            RequestBody body = RequestBody.create(MediaType.parse("application/json"), UPLOAD_SET_BODY);
            ...
            try (Response uploadSetsCallResponse = authClient.newCall(uploadSetRequest).execute()) {
                if (!uploadSetsCallResponse.isSuccessful()) {
                    ...
                    return uploadError(uploadSetsCallResponse, url);
                }
                ...
                JsonObject rootObject = (JsonObject) root;
                String uploadSetId = rootObject.get(ID).getAsString();
                ...
                final URL uploadSetUrl = new URL(url.toString() + "/" + uploadSetId + FILES);
                ...
                try (Response uploadCallResponse = authClient.newCall(uploadRequest).execute()) {
                    if (!uploadCallResponse.isSuccessful()) {
                        ...
                        return uploadError(uploadCallResponse, uploadSetUrl);
                    }
    ...
    @NonNull
    public static List<ImageStoreConfiguraton> getInstances(@NonNull Context context) {
        final ExecutorTask<Void, Void, List<ImageStoreConfiguraton>> getInstancesTask = new ExecutorTask<Void, Void, List<ImageStoreConfiguraton>>() {
            @Override
            protected List<ImageStoreConfiguraton> doInBackground(Void nothing) throws IOException {

                List<ImageStoreConfiguraton> result = new ArrayList<>();
                ...
                try (Response instancesResponse = App.getHttpClient().newBuilder().connectTimeout(TIMEOUT, TimeUnit.MILLISECONDS)
                    ...
                    for (JsonElement e : ((JsonArray) instances)) {
                        ...
                        JsonObject instanceConfiguration = getJsonObject((JsonObject) e, CONFIGURATION_JSON);
                        JsonObject auth = getJsonObject(instanceConfiguration, AUTH);
                        if (!auth.get(ENABLED).getAsBoolean() || !auth.get(REGISTRATION_IS_OPEN).getAsBoolean()) {
                            continue;
                        }
                        JsonObject name = getJsonObject(instanceConfiguration, NAME);
                        JsonElement label = name.get(LABEL);
                        if (instanceUrl.isJsonPrimitive() && label.isJsonPrimitive()) {
                            ImageStoreConfiguraton imageStoreConf = new ImageStoreConfiguraton(null, label.getAsString(), ImageStoreType.PANORAMAX,
     ...
     * @throws IOException
     */
    private static JsonObject getJsonObject(JsonObject parent, String name) throws IOException {
        JsonElement obj = parent.get(name);
        if (obj != null && !obj.isJsonObject()) {
            Log.e(DEBUG_TAG, "Unable to create Panoramax instance list");
