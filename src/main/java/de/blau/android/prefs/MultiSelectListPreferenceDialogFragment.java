    ...
    protected void onPrepareDialogBuilder(AlertDialog.Builder builder) {
        ...
        builder.setMultiChoiceItems(preference.getEntries(), checkedItems, new DialogInterface.OnMultiChoiceClickListener() {
            public void onClick(DialogInterface dialog, int which, boolean isChecked) {
                if (isChecked) {
                    mPreferenceChanged |= mNewValues.add(preference.getEntryValues()[which].toString());
                } else {
                    mPreferenceChanged |= mNewValues.remove(preference.getEntryValues()[which].toString());
                }
            }
        });
        mNewValues.clear();
