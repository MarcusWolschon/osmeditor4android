...
public class AdvancedPrefDatabase extends SQLiteOpenHelper implements AutoCloseable {
    ...
    private void addDefaultImageStoreEntries(@NonNull SQLiteDatabase db) {
        addImageStore(db, ID_PANORAMAX_DEV, ID_PANORAMAX_DEV, ImageStoreType.PANORAMAX, Urls.DEFAULT_PANORAMAX_DEV_UPLOAD_URL, true);
    }
     ...
     */
    @NonNull
    public ImageStoreConfiguraton[] getImageStores() {
     ...
     */
    @NonNull
    public synchronized ImageStoreConfiguraton[] getImageStores(@Nullable String id) {
        SQLiteDatabase db = getReadableDatabase();
        Cursor dbresult = db.query(IMAGESTORES_TABLE, new String[] { ID_COL, NAME_COL, TYPE_COL, URL_COL, ACTIVE_COL }, id == null ? null : WHERE_ID,
                id == null ? null : new String[] { id }, null, null, null);
        ImageStoreConfiguraton[] result = new ImageStoreConfiguraton[dbresult.getCount()];
        dbresult.moveToFirst();
        for (int i = 0; i < result.length; i++) {
            result[i] = new ImageStoreConfiguraton(dbresult.getString(0), dbresult.getString(1), ImageStoreType.valueOf(dbresult.getString(2)),
     ...
     * @return ImageStore[]
     */
    public synchronized ImageStoreConfiguraton[] getActiveImageStores() {
        SQLiteDatabase db = getReadableDatabase();
     ...
     */
    public synchronized void addImageStore(@NonNull String id, String name, ImageStoreType type, String url, boolean active) {
        SQLiteDatabase db = getWritableDatabase();
     ...
     */
    private synchronized void addImageStore(@NonNull SQLiteDatabase db, @NonNull String id, String name, ImageStoreType type, String url, boolean active) {
        ContentValues values = new ContentValues();
        values.put(ID_COL, id);
        values.put(NAME_COL, name);
        values.put(TYPE_COL, type.name());
     ...
     * @param active use this image storeif true
     */
    public synchronized void updateImageStore(@NonNull String id, String name, ImageStoreType type, String url, boolean active) {
        Log.d(DEBUG_TAG, "Setting geocoder " + id + " active to " + active);
        SQLiteDatabase db = getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(NAME_COL, name);
    ...
    private void setState(@NonNull String table, @NonNull String id, boolean active, boolean singleActive) {
        Log.d(DEBUG_TAG, "In table " + table + " setting pref " + id + " active to " + active);
        SQLiteDatabase db = getWritableDatabase();
        ContentValues values = new ContentValues();
        if (singleActive) {
