...
import de.blau.android.imagestore.ImageStore;
import de.blau.android.imagestore.PanoramaxStore;
import de.blau.android.imagestore.UploadResult;
...
public class UploadImage {
    ...
    private static void dialog(@NonNull Context context, @NonNull Preferences prefs, @NonNull ImageAction action, @NonNull File imageFile,
        ...
        try (AdvancedPrefDatabase db = new AdvancedPrefDatabase(context)) {
            final ImageStoreConfiguraton[] configurations = db.getImageStores();
     ...
     * @param alwaysRemove if true always remove the file
     */
    private static void upload(@NonNull Context context, @NonNull Preferences prefs, final boolean remove, @NonNull final ImageStoreConfiguraton configuration,
            @NonNull ImageAction action, @NonNull final File imageFile, boolean alwaysRemove) {

        final ImageStore imageStore = getImageStore(configuration);
    ...
    private static ImageStore getImageStore(ImageStoreConfiguraton configuration) {
        if (ImageStoreType.PANORAMAX == configuration.type) {
            return new PanoramaxStore(configuration);
        }
