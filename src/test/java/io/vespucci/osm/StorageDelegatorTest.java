...
import io.vespucci.osm.ClipboardStorage;
import io.vespucci.osm.DelegatorUtil;
import io.vespucci.osm.MergeAction;
import io.vespucci.osm.MergeIssue;
...
import io.vespucci.osm.PreconditionIssue;
...
import io.vespucci.osm.UpdateFromChanges;
...
public class StorageDelegatorTest {
    ...
    public void cutUndo() {
        ...
        logic.setMap(new io.vespucci.Map(ApplicationProvider.getApplicationContext()), true);
        Way w = DelegatorUtil.addWayToStorage(d, true);
        Way temp = (Way) d.getOsmElement(Way.NAME, w.getOsmId());
        assertNotNull(temp);
