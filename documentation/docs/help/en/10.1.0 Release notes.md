...
Layers provided via OAM can be added to the imagery configuration via a query to the OAM catalog (this currently doesn't work for devices running Android version older than 3.0 as OAM doesn't support TLS 1.0).

### MBTiles

MBTile files containing jpen og png imagery can now be used as custom imagery sources on device.
