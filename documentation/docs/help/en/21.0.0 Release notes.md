...
If you are mapping in a region that has questionable Internet connectivity you have may have experienced the problem that if a upload fails it is unclear if you can retry and if you can't you are left a bit stuck. See [diary post on handling failed uploads](https://www.openstreetmap.org/user/SimonPoole/diary/406218).
...
Hardware rendering is now the default on devices with Android 10 or later. The preference for for this has been removed on older devices and its location has been moved to the miscellaneous category of the _Advanced preferences_.
...
A CSV file with _lat_ and _lon_ columns with WGS94 coordinates can be used to create a GeoJSON layer (the converted file will be stored locally).
