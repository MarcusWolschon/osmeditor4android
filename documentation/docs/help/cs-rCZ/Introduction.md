...
* ** Stáhnout aktuální zobrazení ** - stáhnout oblast viditelnou na obrazovce a sloučit ji s existujícími daty * (vyžaduje připojení k síti) *
* ** Vymazat a stáhnout aktuální zobrazení ** - vymazat všechna data v paměti a poté stáhnout oblast viditelnou na obrazovce * (vyžaduje připojení k síti) *
* ** Nahrát data na server OSM ** - nahrát úpravy do OpenStreetMap * (vyžaduje ověření) * * (vyžaduje připojení k síti) *
* ** Automatické stahování podle polohy ** - automatické stahování oblasti kolem aktuální geografické polohy * (vyžaduje připojení k síti nebo offline data) * * (vyžaduje GPS) *
* ** Automatické stahování a přibližování ** - automatické stahování dat pro aktuálně zobrazenou oblast mapy * (vyžaduje připojení k síti nebo offline data) * * (vyžaduje GPS) *
* ** File ... ** - ukládání a načítání dat OSM do / z souborů zařízení.
* ** Poznámka / chyby ... ** - stáhnout (automaticky a ručně) poznámky OSM a „chyby“ z nástrojů QA (aktuálně OSMOSE) * (vyžaduje připojení k síti) *
...
Nastavení vrstev lze měnit pomocí ovládání vrstev (pravý horní roh), všechna ostatní nastavení jsou dosažitelná pomocí tlačítka předvoleb hlavní nabídky.

* Vrstva pozadí - k dispozici je široká škála leteckých a satelitních snímků na pozadí, výchozí hodnota pro tuto mapu je „standardní styl“ mapy z openstreetmap.org.
* Překryvná vrstva - jedná se o poloprůhledné vrstvy s dalšími informacemi, například stopy GPX. Přidání překryvu může způsobit problémy se staršími zařízeními a například s omezenou pamětí. Výchozí: žádné.
* Zobrazení poznámek / chyb. Otevřené poznámky a chyby se zobrazí jako žlutá ikona chyby, ty uzavřené zelené. Výchozí: zapnuto.
* Foto vrstva. Zobrazuje georeferenční fotografie jako červené ikony fotoaparátu, pokud jsou k dispozici informace o směru, ikona se otočí. Výchozí: vypnuto.
